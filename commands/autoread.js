...
module.exports = {
    ...
    async execute(m, { VoidMD, args, owner, sender }) {
        ...
        if (arg === 'on') {
            if (readListener) return await VoidMD.sendMessage(m.key.remoteJid, { text: 'Autoread already ON 📖' }, { quoted: m });

            await VoidMD.sendMessage(m.key.remoteJid, { text: 'Autoread enabled ✅' }, { quoted: m });
            ...
            if (!readListener) return await VoidMD.sendMessage(m.key.remoteJid, { text: 'Autoread already OFF 💀' }, { quoted: m });
            VoidMD.ev.off('messages.upsert', readListener);
            readListener = null;
            global.autoread = false;
            await VoidMD.sendMessage(m.key.remoteJid, { text: 'Autoread disabled ❌' }, { quoted: m });
